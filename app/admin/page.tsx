  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="bg-white p-6 rounded-lg shadow-md">
          <h2 className="text-xl font-semibold mb-4">Все сделки ({allDeals.length})</h2>
          <div className="space-y-4">
            {allDeals.map(deal => (
              <div key={deal._id} className="border-b pb-2">
                <p className="font-medium">Сделка #{deal._id.slice(0, 6)}</p>
                <p className="text-sm">Пользователь: {deal.userId.slice(0, 6)}...</p>
      ...
      <div className="mt-8 bg-white p-6 rounded-lg shadow-md">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="bg-green-50 p-4 rounded-lg">
            ...
            <p className="text-2xl font-bold">{allDeals.filter(d => !d.is_deleted).length}</p>
          </div>
          <div className="bg-purple-50 p-4 rounded-lg">
            <p className="text-sm text-purple-800">Общая сумма</p>
            <p className="text-2xl font-bold">
              {allDeals.reduce((sum, deal) => sum + (deal.totalAmount || 0), 0).toLocaleString()} ₽
            </p>
          </div>
        </div>
      </div>
