...
export default function EditDealPage() {
  ...
  useEffect(() => {
    const fetchDeal = async () => {
      try {
        const token = getCookie('tg_news_bot_access_token')
        const response = await fetch(`https://appgrand.worldautogroup.ru/deals/${dealId}`, {
          headers: {
  ...
  useEffect(() => {
    const total = Number(amountPurchase) + Number(amountDelivery) + Number(companyProfit)
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-6 bg-white shadow rounded-lg">
      <h1 className="text-xl font-semibold text-gray-800 mb-4">Редактирование сделки</h1>
      <span className="inline-flex items-center gap-x-1.5 py-2 px-3 rounded-full text-xs font-medium border border-blue-600 text-blue-600 dark:text-blue-500">{serviceName.toUpperCase()}</span>
      <h3 className="text-xl font-semibold text-gray-500 mb-4">{serviceName}</h3>
      ...
      <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-6 text-sm leading-relaxed">
        ...
        <div className="flex flex-col">
          <label className="text-gray-600 font-medium mb-2">Стадия сделки</label>
          <select
            value={stageId}
        ...
        <div className="flex flex-col">
          <label className="text-gray-600 font-medium mb-2">Единица измерения</label>
          <input
        ...
        {serviceName === 'продажа сырья' && (
          <div className="flex flex-col">
            <label className="text-gray-600 font-medium mb-2">Способ получения</label>
            <input
        ...
        <div className="flex flex-col">
          <label className="text-gray-600 font-medium mb-2">Адрес отправления</label>
          <select
        ...
        <div className="flex flex-col">
          <label className="text-gray-600 font-medium mb-2">Адрес доставки</label>
          <select
        ...
        {serviceName === 'продажа сырья' && (
          <div className="flex flex-col">
            <label className="text-gray-600 font-medium mb-2">Сумма закупки</label>
            <input
              type="number"
        ...
        <div className="flex flex-col">
          <label className="text-gray-600 font-medium mb-2">Сумма доставки</label>
          <input
            type="number"
        ...
        <div className="flex flex-col">
          <label className="text-gray-600 font-medium mb-2">Надбавка фирме</label>
          <input
            type="number"
            value={companyProfit}
        ...
        <div className="flex flex-col">
          <label className="text-gray-600 font-medium mb-2">Итоговая сумма</label>
          <input
            type="number"
            value={totalAmount}
            disabled
        ...
        <div className="flex flex-col">
          <label className="text-gray-600 font-medium mb-2">Доход менеджеру</label>
          <input
            type="number"
            value={managerProfit}
      ...
      >
        ...
        {saving && <svg className="animate-spin h-5 w-5 ml-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
